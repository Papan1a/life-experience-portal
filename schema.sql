...
CREATE TYPE complexity_level  AS ENUM ('low','medium','high');  -- OPEN: confirm set
...
CREATE TABLE users (
    ...
    bio                text,
    avatar_url         text,                                  -- own-upload key/path (avatar subsystem)
    is_admin           boolean NOT NULL DEFAULT false,
    invited_by_user_id uuid REFERENCES users(id) ON DELETE RESTRICT,
    invite_id          uuid,                                  -- FK added after invites (D8)
    consent_at         timestamptz NOT NULL DEFAULT now(),    -- minimal GDPR posture
...
CREATE TABLE invites (
    ...
    revoked_at  timestamptz,                                   -- admin revoke
    created_at  timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE activities (
    ...
    cost_tier          cost_tier,
    estimated_duration text,                                   -- OPEN: free text vs structured
    requirements       text,
...
CREATE INDEX idx_activities_lower_title ON activities (lower(title));    -- dup-check warning
...
CREATE TABLE variants (
    ...
    updated_at         timestamptz NOT NULL DEFAULT now(),
    deleted_at         timestamptz,
    UNIQUE (id, activity_id)                                   -- referenced by children (D5)
);
...
--   one mutable row per (user, activity, variant); status changes in place.
...
CREATE TABLE user_experiences (
    ...
    created_at  timestamptz NOT NULL DEFAULT now(),
    updated_at  timestamptz NOT NULL DEFAULT now(),
    -- variant must belong to activity (check skipped when variant_id IS NULL)
